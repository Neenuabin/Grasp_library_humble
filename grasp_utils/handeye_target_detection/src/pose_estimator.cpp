...
PoseEstimator::PoseEstimator(std::shared_ptr<rclcpp::Node>& node, std::string param1, std::string pattern, std::string image_topic,
  ...
  , charuco_board_square_size_(charuco_board_square_size)
{
  ...
  if (invalid_dictionary_input) {
        RCLCPP_ERROR(node_->get_logger(), "Invalid dictionary input: %s, default dictionary DICT_6X6_250 used.", dictionary.c_str());
    }
